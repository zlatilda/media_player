...
void Media_thread::get_available_media_windows(const char* dirname)
{
    QDirIterator it(dirname, QStringList() << "*.mp3", QDir::Files, QDirIterator::Subdirectories);
        QDirIterator it1(dirname, QStringList() << "*.mp4", QDir::Files, QDirIterator::Subdirectories);
...
void Media_thread::get_available_media_linux(const char* dirname)
{
    DIR* dir = opendir(dirname);
        ...
        while (entity != NULL) {
            ...
            if(ext == "mp4" || ext == "mp3")
                emit get_available_media_linux(QString::fromStdString(s));
